;;; day7 --- a solution to day 7 -*- lexical-binding: t; -*-
...
(defun parse-program-relationship (line)
  "Parse LINE to a program name, number and sub program relationship."
  (pcase (split-string line "->" t " ")
    (`(,name-and-number ,points-to)
     `(,@(parse-name-and-number name-and-number)
       ,(split-string points-to ", " t)))
    (`(,name-and-number)
     `(,@(parse-name-and-number name-and-number)))))

(defun parse-name-and-number (name-and-number)
  "Parse the name of the disk and it's weight from NAME-AND-NUMBER."
  (pcase (split-string name-and-number " ")
    (`(,name ,number)
     `(,name ,(string-to-number
               (apply #'string
                      (cl-remove ?\)
                                 (cl-remove ?\(
                                            (string-to-list number)))))))))

(parse-program-relationship "ugml (68) -> gyxo, ebii, jptl")

(defun create-tree (relationships)
  "Make a tree from the given RELATIONSHIPS."
  (let ((tree (make-hash-table :test 'equal)))
    (dolist (relationship relationships tree)
      (pcase relationship
        (`(,name ,weight ,points-to)
         (dolist (to points-to)
           (puthash name (let ((val (gethash name tree)))
                           (cons (cons to (car val))
                                 (list weight)))
                    tree)))
        (`(,name ,weight)
         (puthash name (cons (list nil) (list weight)) tree))))))

(defun find-root (tree)
  "Find the root of TREE."
  (car (cl-set-difference
        (hash-table-keys tree)
        (apply (apply-partially #'seq-concatenate 'list)
               (mapcar #'car (cl-remove nil (hash-table-values tree))))
        :test #'equal)))

(defun day7-part-1 (input)
  "Solve day 7 for INPUT."
  (interactive "sInput: ")
  (let* ((input-lines (parse-lines input))
         (instructions (mapcar #'parse-program-relationship input-lines))
         (tree (create-tree instructions)))
    (message
     "%s"
     (find-root tree))))
...
(defun weight-of-sub-tree (tree disk-name)
  "Produce the total weight of TREE from DISK-NAME."
  (pcase (gethash disk-name tree)
    (`(,points-to . ,weight)
     (if (equal points-to (list nil))
         (car weight)
       (+ (car weight)
          (let ((sum 0))
            (dolist (to points-to sum)
              (incf sum (weight-of-sub-tree tree to)))))))))

(let* ((input-lines (parse-lines "pbga (66)
xhth (57)
ebii (61)
havc (66)
ktlj (57)
...
cntj (57)"))
       (instructions (mapcar #'parse-program-relationship input-lines))
       (tree (create-tree-with-weights instructions)))
  (weight-of-sub-tree tree "tknk"))

(defun weight-of-imbalanced-branch (xs)
  "Produce the weight of the odd x out in XS."
  (let ((min (seq-min xs))
        (max (seq-max xs)))
    (if (> (cl-count min xs)
           (cl-count max xs))
...
(index-of-imbalanced-branch '(243 243 251))

(defun find-new-weight (tree)
  "Iterate through TREE to find the point of imbalance then compute the new weight."
  (let* ((root                (find-root tree))
         (sub-nodes           (car (gethash root tree)))
         (weight-of-sub-nodes (mapcar (apply-partially #'weight-of-sub-tree tree)
                                      sub-nodes))
         (weight              (weight-of-imbalanced-branch weight-of-sub-nodes))
         (index               (cl-position weight weight-of-sub-nodes))
         previous-weights)
    (while (and (not (null sub-nodes))
                (not (eq 1 (length (seq-uniq weight-of-sub-nodes)))))
      (setq previous-weights    weight-of-sub-nodes)
      (setq root                (nth index sub-nodes))
      (setq sub-nodes               (car (gethash root tree)))
      (setq weight-of-sub-nodes (mapcar (apply-partially #'weight-of-sub-tree tree)
                                        sub-nodes))
      (setq weight              (weight-of-imbalanced-branch weight-of-sub-nodes))
      (setq index               (cl-position weight weight-of-sub-nodes)))
    (+ (cadr (gethash root tree))
       (- (car (cl-remove (weight-of-imbalanced-branch previous-weights)
                          previous-weights))
          (weight-of-imbalanced-branch previous-weights)))))
...
cntj (57)"))
       (instructions (mapcar #'parse-program-relationship input-lines))
       (tree (create-tree-with-weights instructions)))
  (find-new-weight tree))

(defun day7-part-2 (input)
  "Solve day 7 part 2 for INPUT."
  (interactive "sInput: ")
  (let* ((input-lines (parse-lines input))
         (instructions (mapcar #'parse-program-relationship input-lines))
         (tree (create-tree-with-weights instructions)))
    (message "%s" (find-new-weight tree))))
